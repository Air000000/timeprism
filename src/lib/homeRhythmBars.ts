...
import { currentLocalDayKey, localDayKeyFromDate } from "./time";
...
export function buildHomeRhythmBars(
  homeUsageStack: readonly UsageStackDay[],
  today = new Date(),
): HomeRhythmBar[] {
  const byDay = new Map(homeUsageStack.map((item) => [item.day, item]));
