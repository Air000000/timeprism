...
- `src/api/types.ts`: shared frontend API response/input-adjacent types plus shared usage root-filter and app-rule unions.
...
- `src/lib/settingsPrivacy.ts`: Settings privacy defaults, input event parsing, privacy-save payload, whitelist payload, and whitelist process-name normalization helpers.
...
- `src-tauri/src/services/analytics_params.rs`: analytics limit/day/goal/root-filter normalization, heatmap level, usage-stack include-filter and segment-name helpers, and related characterization tests.
