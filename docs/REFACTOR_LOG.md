...
## 2026-06-09: R-243 Cross-Stack Checkpoint After Weekly-Day Toggle Helper
...
- Confirm that the weekly-day toggle helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-242 reduced `useHomeReminderPanel.ts`.
...
- No runtime behavior changes in this checkpoint.
- Weekly-day toggle helper changes from R-242 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual weekly-day chip interaction smoke testing.
...
## 2026-06-09: R-245 Cross-Stack Checkpoint After Home Goal Progress Helper
...
- Confirm that the Home goal progress helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-244 reduced `useHomeOverview.ts`.
...
- No runtime behavior changes in this checkpoint.
- Home goal progress helper changes from R-244 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Home progress visual smoke testing.
...
## 2026-06-09: R-247 Cross-Stack Checkpoint After Due Reminder Count Helper
...
- Confirm that the due reminder count helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-246 reduced `useHomeOverview.ts`.
...
- No runtime behavior changes in this checkpoint.
- Due reminder count helper changes from R-246 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Home reminder count smoke testing.
...
## 2026-06-09: R-249 Cross-Stack Checkpoint After Insights Metrics Helper
...
- Confirm that the Insights metrics helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-248 reduced `useInsightsData.ts`.
...
- No runtime behavior changes in this checkpoint.
- Insights metrics helper changes from R-248 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Insights view smoke testing.
...
## 2026-06-09: R-251 Cross-Stack Checkpoint After Heatmap Goal Setting Helper
...
- Confirm that the heatmap goal setting helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-250 reduced `useHeatmapGoalSetting.ts`.
...
- No runtime behavior changes in this checkpoint.
- Heatmap goal setting helper changes from R-250 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual heatmap goal slider and persistence smoke testing.
...
## 2026-06-09: R-253 Cross-Stack Checkpoint After Usage Root Filter Type
...
- Confirm that the usage root filter type extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-252 shared the frontend usage root-filter type.
...
- No runtime behavior changes in this checkpoint.
- Usage root filter type changes from R-252 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Insights filter smoke testing.
...
## 2026-06-09: R-257 Cross-Stack Checkpoint After Guard Rule Save Payload Helper
...
- Confirm that the Guard rule-save payload helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-256 reduced repeated payload construction in `useGuardData.ts`.
...
- No runtime behavior changes in this checkpoint.
- Guard rule-save payload helper changes from R-256 remain covered by their existing TypeScript and build checks.
...
## 2026-06-20: R-263 Cross-Stack Checkpoint After Guard Idle Prompt Helper
...
- Confirm that the Guard idle prompt helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-262 moved prompt selection and remember-choice rules into `guardIdle.ts`.
...
- No runtime behavior changes in this checkpoint.
- Guard idle helper changes from R-262 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Guard idle prompt resolution smoke testing.
...
## 2026-06-20: R-265 Cross-Stack Checkpoint After Guard Input Event Helper
...
- Confirm that the Guard input event helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-264 moved Guard DOM event parsing into `guardInputEvents.ts`.
...
- No runtime behavior changes in this checkpoint.
- Guard input event helper changes from R-264 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Guard input interaction smoke testing.
...
## 2026-06-20: R-267 Cross-Stack Checkpoint After Home Reminder Upsert Input Helper
...
- Confirm that the Home reminder upsert input helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-266 moved modal save-input construction into `homeReminderDraft.ts`.
...
- No runtime behavior changes in this checkpoint.
- Home reminder upsert helper changes from R-266 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Home reminder create/edit smoke testing.
...
## 2026-06-20: R-269 Cross-Stack Checkpoint After Home Reminder Drag Data Helper
...
- Confirm that the Home reminder drag data helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-268 moved drag dataTransfer handling into `homeReminderReorder.ts`.
...
- No runtime behavior changes in this checkpoint.
- Home reminder drag data helper changes from R-268 remain covered by their existing TypeScript and build checks.
...
## 2026-06-20: R-272 Analytics Usage Stack Include Helper Extraction
...
- Move usage-stack root-filter include logic into `analytics_params.rs`.
- Keep usage-stack SQL, interval clipping, day buckets, segment aggregation, and result ordering in `analytics.rs`.
- Add focused tests for `ALL`, `LEARN`, and `REST` include behavior before deeper usage-stack refactors.
...
| From | To | Notes |
| --- | --- | --- |
| `analytics.rs` | `analytics_params.rs` | `usage_stack_includes_mapped_type` helper |

Behavior expected to stay the same:

- `ALL` still includes `LEARN`, `REST`, and `IGNORE` mapped rows.
- `LEARN` still includes only `LEARN` mapped rows.
- `REST` still includes only `REST` mapped rows.
- Usage-stack invalid filter validation still happens before the query loop.
- SQL query body, row parsing, interval clipping, day accumulation, segment names, and result ordering are unchanged.
...
- `cargo test analytics_params` passed in `src-tauri` with 6 focused tests.
- `cargo check` passed in `src-tauri`.
- `cargo test` passed in `src-tauri` with 25 tests.
- `git diff --check` passed.
- `git diff --cached --check` passed.

Manual smoke tests:

- Not run for this analytics usage-stack include helper extraction batch.

Risks:

- Usage-stack UI and live analytics output were not manually smoke-tested.
- Validation covered include-filter boundaries and the existing backend test suite, but not live rendering against a real user database.

Rollback:

- Revert this batch to move the usage-stack include `match` expression back into `analytics.rs`.
...
## 2026-06-20: R-277 Cross-Stack Checkpoint After Home Reminder Default Draft Helper
...
- Confirm that the Home reminder default draft helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-276 moved default reminder modal draft construction into `homeReminderDraft.ts`.
...
- No runtime behavior changes in this checkpoint.
- Home reminder default draft helper changes from R-276 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Home reminder modal open/reset smoke testing.
...
## 2026-06-20: R-279 Cross-Stack Checkpoint After Guard Workflow State Helper
...
- Confirm that the Guard workflow state helper extraction does not break Rust/Tauri compile checks.
- Record a clean checkpoint after R-278 moved workflow step state calculation into `guardWorkflowState.ts`.
...
- No runtime behavior changes in this checkpoint.
- Guard workflow state helper changes from R-278 remain covered by their existing TypeScript and build checks.
...
- This checkpoint does not replace manual Guard workflow progression smoke testing.
...
- Commit R-286.
## 2026-06-29: R-287 Guard Feedback Helper Backend Checkpoint
...
- Confirm the backend still compiles after the Guard feedback helper extraction.
...
- This checkpoint does not replace a live Guard UI smoke test.
...
- Commit R-288.
## 2026-06-29: R-289 Home Reminder Feedback Helper Backend Checkpoint
...
- Confirm the backend still compiles after the Home reminder feedback helper extraction.
- Preserve a traceable checkpoint before the next refactor batch.
...
- This checkpoint does not replace a live reminder modal smoke test.
...
- Commit R-290.
## 2026-06-29: R-291 Settings Privacy Feedback Helper Backend Checkpoint
...
- Confirm the backend still compiles after the Settings privacy feedback helper extraction.
- Preserve a traceable checkpoint before the next refactor batch.
...
- This checkpoint does not replace a live Settings privacy smoke test.
...
- Commit R-292.
## 2026-06-29: R-293 Reminder Mutation Runner Backend Checkpoint
...
- Confirm the backend still compiles after the reminder mutation runner extraction.
- Preserve a traceable checkpoint before the next refactor batch.
...
- This checkpoint does not replace a live reminder workflow smoke test.
...
- Commit R-298.
## 2026-06-29: R-299 Home Reminder Panel Action Runner Backend Checkpoint
...
- Confirm the backend still compiles after the home reminder panel action runner extraction.
- Preserve a traceable checkpoint before the next refactor batch.
...
- This checkpoint does not replace a live reminder panel workflow smoke test.
